...
import com.example.greenifyme.data.material.toMaterialOption
...
fun SelectQuantity(model : UserFormModel, state : UserFormState) {
	...
	val typeOfOptions : OptionsType = selectedMaterial.options.toMaterialOption().type

	// If the type is WEIGHT this will be null
	val pointsPerPiece : Int? = selectedMaterial.options.toMaterialOption().pointsPerPiece
	// If the type is PIECE this will be null
	val pointsPerGram : Int? = selectedMaterial.options.toMaterialOption().pointsPerGram
